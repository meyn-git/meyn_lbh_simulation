...
void main() {
  ...
  group('$TurnTableSpeedProfileForOmniaContainers class', () {
    ...
    test('Should travel 90 degrees in 11.5 seconds', () {
      speedProfile
    ...
    test('Should travel 180 degrees in 19 seconds', () {
      speedProfile
          .durationOfDistance(180)
          .should
  ...
  group(
    ...
    test(
        ...
        () {
      speedProfile.maxSpeed.should.beCloseTo(0.252, delta: 0.001);
    });
    test('Acceleration should be 0.252/s / 1.5s = 0.1685 m/s/s', () {
      speedProfile.acceleration.should.beCloseTo(0.168, delta: 0.001);
    });
    test('Deceleration should be 0.252/s / 0.7s = 0.36 m/s/s', () {
      speedProfile.deceleration.should.beCloseTo(0.36, delta: 0.001);
    });

    test('Should travel 2.75m in 12seconds', () {
      speedProfile
          .durationOfDistance(2.75)
          .should
          .be(const Duration(seconds: 12));
    });

    test('Should travel 5.5m in 22.9 seconds', () {
      speedProfile
          .durationOfDistance(5.5)
          .should
          .be(const Duration(milliseconds: 22900));
    });
  });

  group(
      '$ConveyorWithStopperSpeedProfileForModulesWith2CompartmentsOrMorePerLevel class',
      () {
    var speedProfile =
        const ConveyorWithStopperSpeedProfileForModulesWith2CompartmentsOrMorePerLevel();
    test(
        'MaxSpeed should be 2.75m / (0.5*1.5s + (14s-1.5s-4.7s) + 0.5*4.7s) = 0.2525m/s',
        () {
      speedProfile.maxSpeed.should.beCloseTo(0.252, delta: 0.001);
    });
    test('Acceleration should be 0.252/s / 1.5s = 0.1685 m/s/s', () {
    ...
    test('Deceleration should be 0.252/s / 4.7s = 0.36 m/s/s', () {
      speedProfile.deceleration.should.beCloseTo(0.0537, delta: 0.001);
    });

    test('Should travel 2.75m in 14seconds', () {
      speedProfile
          .durationOfDistance(2.75)
          .should
          .be(const Duration(seconds: 14));
    });

    test('Should travel 5.5m in 24.9 seconds', () {
      speedProfile
          .durationOfDistance(5.5)
          .should
          .be(const Duration(milliseconds: 24900));
    });
  });
   ...
   group(
    ...
    test(
        'MaxSpeed should be 2.75m / (0.5*1.5s + (18.7s-1.5s-4.7s) + 0.5*11.3s) = 0.2235m/s',
        () {
      speedProfile.maxSpeed.should.beCloseTo(0.2235, delta: 0.001);
    });
    test('Acceleration should be 0.2235/s / 1.5s = 0.149 m/s/s', () {
      speedProfile.acceleration.should.beCloseTo(0.149, delta: 0.001);
    });
    test('Deceleration should be 0.2235/s / 11.3s = 0.0197 m/s/s', () {
      speedProfile.deceleration.should.beCloseTo(0.0197, delta: 0.001);
    });

    test('Should travel 2.75m in 18.7 seconds', () {
      speedProfile
          .durationOfDistance(2.75)
          .should
          .be(const Duration(milliseconds: 18700));
    });

    test('Should travel 5.5m in 31 seconds', () {
      speedProfile
          .durationOfDistance(5.5)
          .should
          .be(const Duration(seconds: 31));
    });
  });
   ...
   group(
    ...
    test(
        'MaxSpeed should be 2.75m / (0.5*2s + (19s-2s-2s) + 0.5*2s) = 0.1617 m/s',
        () {
      speedProfile.maxSpeed.should.beCloseTo(0.1617, delta: 0.001);
    });
    test('Acceleration should be 0.1617/s / 2s = 0.0808 m/s/s', () {
      speedProfile.acceleration.should.beCloseTo(0.0808, delta: 0.001);
    });
    test('Deceleration should be 0.1617/s / 2s = 0.0808 m/s/s', () {
      speedProfile.deceleration.should.beCloseTo(0.0808, delta: 0.001);
    });

    test('Should travel 2.75m in 19 seconds', () {
      speedProfile
          .durationOfDistance(2.75)
          .should
          .be(const Duration(seconds: 19));

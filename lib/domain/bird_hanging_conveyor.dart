...
class BirdHangingConveyor extends ActiveCell {
  ...
  BirdHangingConveyor({
    ...
    required this.shacklesPerHour,
  }) : timePerBird=Duration(microseconds: (hourInMicroSeconds/shacklesPerHour).round()), super(
          layout,
          position,
        );
  ...
  BirdBuffer _findBirdBuffer() {
    for (var neighbourDirection in CardinalDirection.values){
      var neighbour=layout.neighbouringCell(this, neighbourDirection);
      if (neighbour is BirdBuffer && (neighbour as BirdBuffer).birdDirection==neighbourDirection.opposite) {
        return neighbour as BirdBuffer;
      }
    }
    throw Exception ('Layout error: ${this.name} must connect to a $BirdBuffer (e.g. a $ModuleTilter)');
  }
...
class ShackleLine {
  ...
  double get lineEfficiency => numberOfShackles==0?0:numberOfBirds/numberOfShackles;
